import { Group, Line, SVG, Shape, SVGElement } from "introcs/graphics";
import { Rectangle, Text, Color, Stroke, Font } from "introcs/graphics";
import { Circle } from "introcs/graphics";

export let svg = (id: string) => new SVG(id);

export let group = (): Group => new Group();

export let line = (x1: number, y1: number, x2: number, y2: number): Line => {
    return new Line(x1, y1, x2, y2);
};

export let rectangle = (width: number, height: number, x: number, y: number) => {
    return new Rectangle(width, height, x, y);
};

export let circle = (radius: number, cx: number, cy: number) => new Circle(radius, cx, cy);

export let add = (g: Group, s: SVGElement): void => {
    g.add(s);
};
...
export let grid = (xMin: number, yMin: number, xMax: number, yMax: number, ticks: number): Grid => {
    return new Grid(xMin, yMin, xMax, yMax, ticks);
}